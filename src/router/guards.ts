...
export const authGuard = async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
  ...
  // Approval gate: block access to protected routes until approved
  if (isLoggedIn && to.meta.requiresAuth && to.path !== "/pending-approval") {
    try {
      const authStore = useAuthUserStore();
      const currentUserResult = await authStore.getCurrentUser();

      const approved = currentUserResult.user?.user_metadata?.approved;
      if (approved === false) {
        return next("/pending-approval");
      }
    } catch (error) {
      console.error("Error checking approval status:", error);
      // If approval check fails, don't block navigation.
    }
  }
